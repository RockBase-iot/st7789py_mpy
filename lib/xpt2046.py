...
class Touch(object):
    ...
    def __init__(self, spi, cs, int_pin=None, int_handler=None,
        ...
        Args:
            ...
            z_threshold (int): Z1 threshold for touch detection (default: 100)
        """
        self.spi = spi
        self.cs = cs
    ...
    def get_touch(self, timeout=2):
        """Take multiple samples to get accurate touch reading.

        Args:
            timeout (int/float): Maximum time in seconds to wait for a valid touch.
        """
        # use provided timeout (default 2 seconds)
        confidence = 5
    ...
    def raw_touch(self):
        ...
        Returns:
            tuple(int, int): X, Y
        """
        # Check Z1 first to detect if screen is actually touched.
        # When not touched, Z1 is near zero; when touched, Z1 increases significantly.
